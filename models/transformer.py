...
class Network(nn.Module):
    def __init__(self, input_size:int, output_size:int, hidden_size:int=512, num_layers:int=5, dropout:float=0.01, no_heads:int=8, maxlen:int=1000):
        ...
        self.posenc = PositionalEncoding(hidden_size, maxlen)
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=hidden_size, nhead=no_heads, dim_feedforward=hidden_size, dropout=0.)
        self.encoder = nn.TransformerEncoder(self.encoder_layer, num_layers)
        self.outlayer = nn.Linear(hidden_size, output_size)
        # dropout is only added over the target layers:
    ...
    def forward(self, x):
        ...
        # we apply the dropout only on the target layers:
        x[:,:,self.outlayer.weight.shape[0]:] = self.dropout(x[:,:,self.outlayer.weight.shape[0]:])
        # forward flow:
        h = self.inlayer(x)
